...
-- =========================================================
-- Fairway Stats HQ realistic demo-team seed
--
-- Restricted to:
-- Team ID: 4858e5d6-d3ae-4aea-a639-59eadb863f9a
-- Team: Fairway Stats Demo Team
--
-- Preserves:
-- - Team and branding
-- - Nick Bayley admin profile
-- - Mason Cole player profile/login
--
-- Replaces:
...
declare
  target_team_id constant uuid :=
    '4858e5d6-d3ae-4aea-a639-59eadb863f9a';
  target_season_id constant uuid :=
    'cea28403-b3e6-46e5-a447-767cc84fad85';
  target_admin_id constant uuid :=
    '048b770b-b685-44b9-8d96-bab59f218afe';
  mason_player_id constant uuid :=
    'b59f11a4-16e6-4908-b2ca-330bfb4b2d2d';
  mason_profile_id constant uuid :=
    '2fbb5f1c-9f84-474a-813e-4e520c69dad6';
begin
  if not exists (
    select 1
    from public.teams
    where id = target_team_id
      and name = 'Fairway Stats Demo Team'
      and school_name = 'Fairway Stats High School'
  ) then
    raise exception
      'Safety check failed: exact demo team was not found.';
  end if;
  ...
  if not exists (
    ...
    where id = target_season_id
      ...
      and name = '2026 Spring Season'
  ) then
    raise exception
      'Safety check failed: expected demo season was not found.';
  end if;
  ...
  if not exists (
    ...
    where id = target_admin_id
      and team_id = target_team_id
      and email = 'nicklaus.bayley@gmail.com'
      and role = 'admin'
  ) then
    raise exception
      'Safety check failed: expected demo administrator was not found.';
  end if;

  if not exists (
    select 1
    from public.players
    where id = mason_player_id
      and team_id = target_team_id
      and profile_id = mason_profile_id
  ) then
    raise exception
      'Safety check failed: Mason Cole linked player was not found.';
  end if;
  ...
  if exists (
    ...
    where team_id = target_team_id
      ...
      and id <> mason_player_id
  ) then
    raise exception
      'Safety check failed: an unexpected linked player exists.';
  end if;
end
$$;
...
-- Keep the existing login-connected player clean and consistent.
...
set
  ...
  status = 'active'
where id = 'b59f11a4-16e6-4908-b2ca-330bfb4b2d2d'
  and team_id = (
    select team_id from demo_context
  );
...
values
  (
    '04263aea-1047-40f3-b9c1-2ee73c4687e7',
    'Avery',
    'Ellis',
    2026,
    1,
  ...
  (
    '943deef9-0c03-40ac-be6a-a8a4c90a2e4e',
    'Sofia',
    'Ramirez',
    2027,
    3,
  ...
  (
    '0298d090-ce62-45de-bee0-ce611fe92e04',
    'Nora',
    'Brooks',
    2028,
    4,
  ...
  (
    '53c198f1-b4f4-4804-a5d6-299a5aed8d4e',
    'Caleb',
    'Price',
    2028,
    5,
  ...
  (
    '23769ad0-68ba-4e77-a48e-4a2838dfa985',
    'Eli',
    'Turner',
    2029,
    6,
