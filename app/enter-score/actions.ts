...
export async function submitRound(
  ...
  if (isMissingText(input.teamId)) {
    return { success: false, message: "Missing team context." };
  }

  if (isMissingText(input.playerId)) {
    return { success: false, message: "Please choose a player." };
  }

  if (isMissingText(input.playedOn)) {
    return { success: false, message: "Please enter the played date." };
  }

  if (input.holes !== 9 && input.holes !== 18) {
    return { success: false, message: "Holes must be 9 or 18." };
  }

  if (!isNumber(input.score) || input.score <= 0) {
    return { success: false, message: "Score is required and must be greater than 0." };
  }
  ...
  if (nonNegativeError) {
    return { success: false, message: nonNegativeError };
  }
  ...
  if (
    ...
    return {
      success: false,
      message: "Fairways hit cannot exceed fairways possible."
    };
  }
  ...
  if (
    ...
    return {
      success: false,
      message: "Greens in regulation cannot exceed GIR possible."
    };
  }
  ...
  try {
    ...
    if (seasonError) {
      return {
        success: false,
        message: `Could not find active season: ${seasonError.message}`
      };
    }
    ...
    if (error) {
      return {
        success: false,
        message: `Could not save round: ${error.message}`
      };
    }
    ...
    return { success: true, message: "Round submitted successfully." };
  } catch (error) {
    return {
      success: false,
      message:
